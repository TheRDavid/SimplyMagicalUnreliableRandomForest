...
public class RForestUI<T extends Comparable<T>> extends JFrame
{
	...
	private JMenuItem graphAllItem = new JMenuItem("Full Graph");
	private JMenuItem splitItem = new JMenuItem("All Splits");
	private JPopupMenu treePopup = new JPopupMenu();
	private JMenuItem treeDataItem = new JMenuItem("Graph Tree Data");
	private RNode selectedNode = null;
	private RTree selectedTree = null;
	Class type;

	public RForestUI(final RForest<T> forest)
	{
		treePopup.add(treeDataItem);
		treeDataItem.addActionListener(new ActionListener()
		{
		...
		graphItem.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent arg0)
			{
				...
				{
					new DataGraph(selectedNode.getTree().getSample().getElements(), null,0,-1, randomForest.getData().getElements());
				}
		...
		{
			DefaultMutableTreeNode treeNode = new DefaultMutableTreeNode(rt);
		...
		tree.addTreeSelectionListener(new TreeSelectionListener()
		{
			public void valueChanged(TreeSelectionEvent e)
			{
				...
				{
					if (!(node.getUserObject() instanceof RTree))
						System.out.println("Node is no RTree");
					else
					{
						selectedTree = (RTree) node.getUserObject();
					}
		...
		tree.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseReleased(MouseEvent arg0)
			{
				if (arg0.getButton() == 3)
				{
					System.out.println("Node: " + selectedNode + " Tree: " + selectedTree);
					...
					{
						treePopup.show(RForestUI.this, arg0.getX(), arg0.getY());
					}
				}
			}
		});
		System.out.println("Done building TreeUI");
		add(new JScrollPane(tree, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS),
				BorderLayout.CENTER);
		...
		if (node.getLeft() != null)
		{
			...
			DefaultMutableTreeNode leftTreeNode = new DefaultMutableTreeNode(
					"LEFT Feature: " + node.getSplitPoint().getFeatureIndex() + " <"
							+ node.getSplitPoint().getSplitValue() + " => " + node.getLeft().getCategory());
			...
			DefaultMutableTreeNode rightTreeNode = new DefaultMutableTreeNode(
					"RIGHT Feature: " + node.getSplitPoint().getFeatureIndex() + " >="
							+ node.getSplitPoint().getSplitValue() + " => " + node.getRight().getCategory());
	...
	class DataGraph extends JDialog
	{
		...
		private JPanel canvas = new JPanel()
		{
			protected void paintComponent(java.awt.Graphics arg0)
			{
				...
				if (type.equals(Double.class))
				{
					...
					g.drawString(
							String.format("%.2f", mouseX / getWidth() * rangeX + smallestX) + " x "
					...
					if (elementsAll != null)
					{
						...
						for (Element<T> a : elementsAll)
						{
							...
							int y = (int) Math
									.round(((e.getAttribute(yFeatureIndex) - smallestY) / rangeY * getHeight()));
							int c = e.getCategory();
		...
		private ArrayList<Element<T>> elementsAll;
	...
	class SplitFinder extends JDialog
	{
		...
		public SplitFinder(RTree.RNode n)
		{
			...
			for (SplitResult<Double> sr : allResults)
			{
				...
				DefaultMutableTreeNode leftDataNode = new DefaultMutableTreeNode(
						"LEFT [" + sr.getLeftSet().getElements().size() + "]");
